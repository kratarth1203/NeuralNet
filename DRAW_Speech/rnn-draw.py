...
def build_rnn(n_visible = 784, n_z = 100, n_hidden_recurrent = 200, T_ = 10, batch_size = 5, sigma_b = 1e-3):
    ...
    #prior LSTM params    
    bi_prior = shared_zeros(n_hidden_recurrent)
    ...
    bf_prior = shared_zeros(n_hidden_recurrent)
    ...
    bc_prior = shared_zeros(n_hidden_recurrent)
    
    bo_prior = shared_zeros(n_hidden_recurrent)
    ...
    def prior_recurrence(h_tm1_prior, c_tm1_prior, v_prior):
        ...
        non_seq = [v_prior, bi_enc, Wci_enc,  Whi_enc, Wvi_enc, bf_enc, Wcf_enc, Whf_enc, Wvc_enc, Wvf_enc,
        Whc_enc, bc_enc, bo_enc, Wco_enc, Who_enc, Wvo_enc, bi_dec, Wci_dec, Whi_dec,
        Wzi_dec, bf_dec, Wcf_dec, Whf_dec, Wzf_dec, Wzc_dec, Whc_dec, bc_dec, bo_dec,
        Wco_dec, Who_dec, Wzo_dec, Wh_enc_mu_z,  Wh_enc_sig_z, Wh_dec_mu_x, Wh_dec_sig_x, b_mu_z, b_sig_z, b_mu_x, b_sig_x]
        ...
        (h_t_enc, h_t_dec, c_t_enc, c_t_dec,  mu_z_t, sigma_z_t, mu_x_t , sigma_x_t ), _ = theano.scan(
            ...
            sequences=[rand_samples_z, rand_samples_x], outputs_info=[h0_enc, h0_dec, c0_enc, c0_dec,  mu_z_0, sigma_z_0,  mu_x_0, sigma_x_0], non_sequences=non_seq)
        ...
        return h_t_prior, c_t_prior, mu_prior_w, sigma_prior_w, mu_z_t, sigma_z_t, mu_x_t, sigma_x_t
...
class Rnn:
    ...
    def __init__(
        ...
        self.r = r
        self.T_ = T_
        (v, cost, monitor, params, updates_train, mu_x_t,  g_mu_x_t, g_sigma_x_t,
    ...
    def train(self,  batch_size=9, num_epochs=4000):
    ...
    def generate(self, filename, show=True):
        ...
        for i in range(n_samples_to_gen):
            ...
            f = open('sample_gauss_cond_onom_%i.pkl' % i, 'w')
            cPickle.dump(g,f)
            f.close()

def test_rnnrbm(batch_size=9, num_epochs=4000):
