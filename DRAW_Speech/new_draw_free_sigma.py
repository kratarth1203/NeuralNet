...
def build_rnn(n_visible = 784, n_z = 100, n_hidden_recurrent = 200, T_ = 10, batch_size = 5, sigma_bound = 1e-3):
    ...
    b_mu_x = shared_zeros(n_visible)
    b_sig_x = shared_zeros(n_visible)
    ...
    Wco_dec, Who_dec, Wzo_dec, Wh_enc_mu_z,  Wh_dec_mu_x,  b_mu_z, b_sig_z, b_mu_x, b_sig_x]
    ...
    def recurrence( sample_z_t, sample_x_t, h_tm1_enc, h_tm1_dec, c_tm1_enc, c_tm1_dec,  mu_z_t,  mu_x_tm1,   v):
        v_hat = v -  ( mu_x_tm1 + (T.exp(b_sig_x) * sample_x_t.reshape((batch_size, n_visible)) ) )#error input
        ...
        return [ h_t_enc, h_t_dec, c_t_enc, c_t_dec,  mu_z_t,  mu_x_t ]
    ...
    (h_t_enc, h_t_dec, c_t_enc, c_t_dec,  mu_z_t, mu_x_t  ), updates_train = theano.scan(
        lambda sample_z_t, sample_x_t, h_tm1_enc, h_tm1_dec, c_tm1_enc, c_tm1_dec, mu_z_t, mu_x_tm1,  v, *_: recurrence( sample_z_t, sample_x_t, h_tm1_enc, h_tm1_dec, c_tm1_enc, c_tm1_dec,  mu_z_t, mu_x_tm1,  v),
        sequences=[rand_samples_z, rand_samples_x], outputs_info=[h0_enc, h0_dec, c0_enc, c0_dec,  mu_z_0, mu_x_0], non_sequences=non_seq)

   
    L_z = (
        prior_log_sigma_z - b_sig_z
        + 0.5 * (
        T.exp(2 * b_sig_z) + (mu_z_t - prior_mu_z) ** 2
        ) / T.exp(2 * prior_log_sigma_z)
        - 0.5
    ...
    L_x  = 0.5 * T.sum( (T.sqr(v - mu_x_t[-1,:,:]) * T.exp(-2*b_sig_x)) + (2*b_sig_x) + T.log(2 * numpy.pi) , axis=1)
    cost = (L_z + L_x).mean()
    monitor = L_x.mean()
    ...
    return (v, cost, monitor, params, updates_train, mu_x_t[-1,:,:],  g_mu_x_t[-1,:,:],
            updates_generate)
...
class Rnn:
    ...
    def __init__(
        ...
        self.r = r
        self.T_ = T_
        (v, cost, monitor, params, updates_train, mu_x_t,  g_mu_x_t,
